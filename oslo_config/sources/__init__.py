...
class ConfigurationSourceDriver(metaclass=abc.ABCMeta):
    ...
    def open_source_from_opt_group(
        ...
        :param conf: The active configuration option handler from which to seek
...
class ConfigurationSource(metaclass=abc.ABCMeta):
    ...
    def get(
        ...
        :returns: A tuple ``(value, location)`` where ``value`` is either the
            option value or ``oslo_config.sources._NoValue`` if the ``(group,
            option)`` is not present in the source, and ``location`` is a
            ``LocationInfo``.
        """
