...
class ConfigFilePartialReloadTestCase(BaseTestCase):

    def setUp(self):
        super(ConfigFilePartialReloadTestCase, self).setUp()
        self.conf.limit_mutation = True

    def _test_conf_files_reload(self):
        paths = self.create_tempfiles([('1',
                                        '[DEFAULT]\n'
                                        'foo = old_foo\n'
                                        '[group]\n'
    ...
    def test_conf_files_reload_none(self):
        """Test that immutable opts are not reloaded

        Also check that the hook is not called if there is nothing to do.
        """
    ...
    def test_conf_files_reload_group(self):
        """Test that mutable opts in groups can be reloaded."""
        def hook(conf, fresh):
            self.assertEqual(self.conf, conf)
            self.assertEqual({('group', 'boo'):
                              {'cli': True, 'value': 'new_boo'}}, fresh)
    ...
    def test_conf_files_reload_naughty_hook(self):
        """Test that the hook can't modify the 'fresh' arg."""
        def hook(conf, fresh):
            fresh.clear()
